...
{
	$sql = "SELECT COUNT(id) FROM tll_users WHERE lang_have_1 = '$langid' OR lang_have_2 = '$langid' " ;
...
{
	$sql = "SELECT COUNT(id) FROM tll_users WHERE lang_want_1 = '$langid' OR lang_want_2 = '$langid' " ;
	$result = mysql_query($sql) ;
	$row = mysql_fetch_row($result) ;
	$k = $row[0] ;
	return $k ;
